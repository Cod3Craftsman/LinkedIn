  ...
  return (
    <div className="w-full h-[80px] bg-[white] fixed top-0 shadow-lg flex justify-between md:justify-around items-center px-[10px] left-0 z-[80]">
      <div className="flex justify-center items-center gap-[10px]">
        ...
        {searchData.length > 0 && <div className="absolute top-[90px] min-h-[50px] h-[auto] lg:left-[20px] left-[0px] shadow-lg rounded-sm w-[100%] lg:w-[700px] bg-white flex flex-col gap-[20px] p-[20px] overflow-auto">
          {searchData.map((sea) => (
            <div className="flex gap-[20px] items-center border-b-2 border-b-gray-300 p-[10px] hover:bg-gray-200 cursor-pointer rounded-lg" key={sea.userName} onClick={() => { handleGetProfile(sea.userName); setActiveSearch(false); setSearchInput(""); setSearchData([]) }}>
        ...
        <form className={`w-[190px] lg:w-[350px] h-[40px] bg-[#f0efe7] lg:flex items-center gap-[10px] px-[10px] py-[5px] rounded-md ${!activeSearch ? "hidden" : "flex"} `} onSubmit={(e) => e.preventDefault()}>
          <div><IoSearchSharp className="w-[23px] h-[23px] text-gray-600" /></div>
          <input type="text" className="w-[80%] h-full bg-transparent outline-none border-0" placeholder="Search users..." value={searchInput} onChange={(e) => setSearchInput(e.target.value)} />
        </form>
      </div>
