    ...
    <div className="w-full h-screen bg-[white] flex flex-col items-center justify-start gap-[10px]">
      ...
      <form className="w-[90%] max-w-[400px] h-[600px] md:shadow-xl flex flex-col justify-center gap-[10px] p-[15px]" onSubmit={handleSignIn}>
        ...
        <div className="w-[100%] h-[50px] border-2 border-gray-600 text-gray-800 text-[18px] rounded-md relative">
          ...
          <span className="absolute right-[20px] top-[8px] cursor-pointer text-[#24b2ff] font-semibold" onClick={() => setShow(prev => !prev)}>{show ? "hide" : "show"}</span>
        </div>

        {err && <p className="text-center text-red-500">
          *{err}
        </p>}
        <button className="w-[100%] h-[50px] rounded-full bg-[#2a9bd8] mt-[40px] text-white" disabled={loading}>{loading ? "Loading..." : "Sign in"}</button>
